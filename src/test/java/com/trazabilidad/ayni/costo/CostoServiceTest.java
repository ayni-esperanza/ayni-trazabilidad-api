...
class CostoServiceTest {
    ...
    @Test
    void actualizarTipoMaterialNoModificaCostosPreviamenteRegistrados() {
        ...
        verify(costoMaterialRepository, never()).saveAll(any());
        verify(proyectoLifecycleService).marcarProyectoComoModificado(1L);
    }
    @Test
    void eliminarOficioGlobalAunqueHayaSidoUsado() {
        Proyecto proyecto = proyecto(1L);
        ...
        costoService.eliminarOficioManoObra(1L, 7L);

        verify(costoManoObraOficioRepository).delete(oficio);
        verify(proyectoLifecycleService).marcarProyectoComoModificado(1L);
    }
