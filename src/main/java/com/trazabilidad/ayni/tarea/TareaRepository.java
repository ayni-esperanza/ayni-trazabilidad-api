...
public interface TareaRepository extends JpaRepository<Tarea, Long>, JpaSpecificationExecutor<Tarea> {
         ...
         */
        @Query(value = """
                        SELECT DISTINCT t.* FROM tareas t
                        LEFT JOIN etapas_proyecto ep ON ep.id = t.etapa_proyecto_id
                        LEFT JOIN proyectos p ON p.id = ep.proyecto_id
                        LEFT JOIN usuarios r ON r.id = t.responsable_id
                        WHERE (CAST(:search AS text) IS NULL OR LOWER(t.titulo) LIKE LOWER(CONCAT('%', CAST(:search AS text), '%')))
                        AND (CAST(:estado AS text) IS NULL OR t.estado = CAST(:estado AS text))
                        AND (CAST(:prioridad AS text) IS NULL OR t.prioridad = CAST(:prioridad AS text))
                        AND (:responsableId IS NULL OR t.responsable_id = :responsableId)
                        AND (:proyectoId IS NULL OR p.id = :proyectoId)
                        """, countQuery = """
                        SELECT COUNT(DISTINCT t.id) FROM tareas t
                        LEFT JOIN etapas_proyecto ep ON ep.id = t.etapa_proyecto_id
                        LEFT JOIN proyectos p ON p.id = ep.proyecto_id
                        WHERE (CAST(:search AS text) IS NULL OR LOWER(t.titulo) LIKE LOWER(CONCAT('%', CAST(:search AS text), '%')))
                        AND (CAST(:estado AS text) IS NULL OR t.estado = CAST(:estado AS text))
